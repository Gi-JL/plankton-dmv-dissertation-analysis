...
dce_data <- read.csv2("./data/DMV data_EN.csv") %>%
  rename(RID = ID)
...
ind_stat <- read.csv2("./data/Pro_contra_statements_respondents_csv_UTF_8.csv") %>%
  filter(!is.na(RID))
...
dce_data <- dce_data %>%
  left_join(ind_stat_merged, by = "RID") %>%
  mutate(
    Pro = replace_na(Pro, 0),
    Contra = replace_na(Contra, 0),
    net_i = Pro - Contra
  ) %>%
  group_by(WS_ID) %>%
  mutate(
    WS_policy_tend = sum(Pro, na.rm = TRUE) - sum(Contra, na.rm = TRUE),
    LOO_tend = WS_policy_tend - net_i
  ) %>%
  ungroup()
...
choice_vars <- paste0("CS", 1:16)
stopifnot(all(choice_vars %in% names(dce_data)))
...
dce_data <- dce_data %>%
  mutate(
    ...
    `Net income group_n` = factor(`Net income group_n`, levels = c(1, 2, 3)),
    BAU_not_chosen = rowSums(across(all_of(choice_vars), ~ .x < 3), na.rm = TRUE)
  )

stopifnot(all(dce_data$BAU_not_chosen >= 0 & dce_data$BAU_not_chosen <= 16))

# Define model formulas.
model_formulas <- list(
  model1 = cbind(BAU_not_chosen, 16 - BAU_not_chosen) ~
    LOO_tend + biospheric_c + `Net income group_n`,

  model2 = cbind(BAU_not_chosen, 16 - BAU_not_chosen) ~
    LOO_tend + egoistic_c + `Net income group_n`,

  model3 = cbind(BAU_not_chosen, 16 - BAU_not_chosen) ~
    LOO_tend + hedonic_c + `Net income group_n`,

  model4 = cbind(BAU_not_chosen, 16 - BAU_not_chosen) ~
    LOO_tend + altruistic_c + `Net income group_n`,

  model5 = cbind(BAU_not_chosen, 16 - BAU_not_chosen) ~
    LOO_tend + biospheric_c + egoistic_c + hedonic_c + altruistic_c +
    `Net income group_n`
)
...
overdisp_fun <- function(model) {
  # Extract Pearson residuals from the fitted binomial model. These residuals
  # compare observed and fitted values, scaled by the variance expected under
  # the binomial model.
  ...
  # Calculate the dispersion ratio. Values substantially above 1 indicate
  # overdispersion and suggest that standard binomial standard errors are too
  # small.
  ratio <- pearson_chisq / model$df.residual
...
overdisp_results <- lapply(binom_models, overdisp_fun)
overdisp_results
...
design <- design %>%
  rename(choiceset = Choice.situation)
...
dce_answers <- dce_data[choice_vars]
other_info <- dce_data[, !(names(dce_data) %in% choice_vars)]
...
dce_data <- dce_data[order(dce_data[, 1], dce_data[, 2]), ]
...
apollo_control <- list(
  modelName = "Clogit_biospheric",
  modelDescr = "Conditional logit in WTP-space: biospheric values",
  indivID = "RID",
  mixing = FALSE
)

apollo_beta <- c(
  b0     = -77.3839,  # ASC for BAU (alt3)
  b0_LOO = 0,         # interaction: BAU ASC x LOO_tend
  b0_BV  = 0,         # interaction: BAU ASC x biospheric_c
  b10    = 56.3098,   # plankton stable and fewer blooms
  b11    = 12.4279,   # plankton less stable and fewer blooms
  b2     = 0.2863,    # carbon sequestration
  b30    = 32.9715,   # fully protected MPA
  b31    = 38.0468,   # highly protected MPA
  b4     = -0.0150    # cost parameter for WTP-space scaling
)
...
rm(list = ls()[!ls() %in% c("model1", "database")])
...
apollo_control <- list(
  modelName = "Clogit_altruistic",
  modelDescr = "Conditional logit in WTP-space: altruistic values",
  indivID = "RID",
  mixing = FALSE
)
...
apollo_beta <- c(
  b0     = -77.3839,
  b0_LOO = 0,
  b0_AV  = 0,         # interaction: BAU ASC x altruistic_c
  b10    = 56.3098,
  b11    = 12.4279,
  b2     = 0.2863,
  b30    = 32.9715,
...
rm(list = ls()[!ls() %in% c("model1", "model2", "database")])
...
apollo_control <- list(
  modelName = "Clogit_hedonic",
  modelDescr = "Conditional logit in WTP-space: hedonic values",
  indivID = "RID",
  mixing = FALSE
)
...
apollo_beta <- c(
  b0     = -77.3839,
  b0_LOO = 0,
  b0_HV  = 0,         # interaction: BAU ASC x hedonic_c
  b10    = 56.3098,
  b11    = 12.4279,
  b2     = 0.2863,
  b30    = 32.9715,
...
rm(list = ls()[!ls() %in% c("model1", "model2", "model3", "database")])
...
apollo_control <- list(
  modelName = "Clogit_egoistic",
  modelDescr = "Conditional logit in WTP-space: egoistic values",
  indivID = "RID",
  mixing = FALSE
)
...
apollo_beta <- c(
  b0     = -77.3839,
  b0_LOO = 0,
  b0_EV  = 0,         # interaction: BAU ASC x egoistic_c
  b10    = 56.3098,
  b11    = 12.4279,
  b2     = 0.2863,
  b30    = 32.9715,
...
rm(list = ls()[!ls() %in% c("model1", "model2", "model3", "model4", "database")])
...
dce_data <- dce_data %>%
  group_by(WS_ID) %>%
  mutate(
    biospheric_WS_sum = sum(biospheric, na.rm = TRUE),
    biospheric_WS_nobs = sum(!is.na(biospheric)),

    biospheric_group_mean_LOO = case_when(
      is.na(biospheric) ~ NA_real_,
      biospheric_WS_nobs < 2 ~ NA_real_,
      TRUE ~ (biospheric_WS_sum - biospheric) / (biospheric_WS_nobs - 1)
    ),

    biospheric_in_group_diff_LOO = abs(biospheric - biospheric_group_mean_LOO)
  ) %>%
  ungroup() %>%
  dplyr::select(
    -biospheric_WS_sum,
...
dce_data <- dce_data %>%
  ...
  group_by(WS_ID) %>%
  mutate(
    WS_policy_pro = sum(Pro, na.rm = TRUE),
    WS_policy_contra = sum(Contra, na.rm = TRUE),
    WS_policy_pro_LOO = WS_policy_pro - Pro,
    WS_policy_contra_LOO = WS_policy_contra - Contra,
    WS_rel_diff_LOO = if_else(
      WS_policy_pro_LOO + WS_policy_contra_LOO > 0,
      abs(WS_policy_pro_LOO - WS_policy_contra_LOO) /
        (WS_policy_pro_LOO + WS_policy_contra_LOO),
      NA_real_
    )
  ) %>%
  ungroup()
...
dce_data <- dce_data %>%
  mutate(Q17.2 = factor(Q17.2, ordered = TRUE))

# Fit ordered logistic regression models.
model1 <- MASS::polr(
  Q17.2 ~ biospheric_c + biospheric_in_group_diff_LOO + WS_rel_diff_LOO,
  data = dce_data,
  Hess = TRUE
)
...
apollo_control <- list(
  modelName = "Clogit_PLN",
  modelDescr = "Conditional logit in WTP-space used for starting-value search",
  indivID = "RID",
  mixing = FALSE
)
